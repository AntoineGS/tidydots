...
Alternatively, override the repository selected by app config by passing the `--dir` flag on every command. The TUI loads hostname choices from the selected repository:
